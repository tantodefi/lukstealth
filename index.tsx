...
const initializeUPProvider = async (): Promise<any> => {
  console.log('🔄 Initializing LUKSO UP provider...');
  
  if (typeof window.lukso !== 'undefined') {
    try {
      // Create the client UP provider
      const provider = createClientUPProvider();
      console.log('✅ LUKSO UP provider initialized:', provider);
      return provider;
    } catch (error) {
    ...
    console.log('❌ LUKSO UP provider not available');
    return null;
  }
};
...
const checkForLuksoProvider = () => {
  ...
  if (typeof window.lukso !== 'undefined') {
    console.log('✅ LUKSO UP provider detected:', window.lukso);
    
    // Log LUKSO provider details
    try {
      const luksoVersion = window.lukso.version || 'Unknown';
      console.log(`LUKSO UP Provider Version: ${luksoVersion}`);
      
      // Check if it has isConnected property
      if ('isConnected' in window.lukso) {
        console.log(`LUKSO UP Connected: ${window.lukso.isConnected}`);
      }
      
      // Check for chainId
      if ('chainId' in window.lukso) {
        console.log(`LUKSO Chain ID: ${window.lukso.chainId}`);
      }
    } catch (error) {
      console.error('Error getting LUKSO UP provider details:', error);
    }
    
    return true;
  } else {
    console.log('❌ LUKSO UP provider not detected');
  }
  
  if (typeof window.ethereum !== 'undefined') {
    console.log('✅ Standard Ethereum provider detected:', window.ethereum);
    ...
    try {
      ...
      console.error('Error getting standard provider details:', error);
    }
  } else {
    console.log('❌ Standard Ethereum provider not detected');
  }
...
const getUPAccounts = async (): Promise<string[]> => {
  if (typeof window.lukso !== 'undefined') {
    try {
      ...
      console.error('Error getting LUKSO UP accounts:', error);
      return [];
    }
  }
  return [];
...
const getUPControllerInfo = async (upAddress: string): Promise<any[]> => {
  if (typeof window.lukso !== 'undefined' && upAddress) {
    console.log('Requesting UP controllers for address:', upAddress);
    
    try {
      // Try up_getControllers method
      const controllers = await window.lukso.request({ method: 'up_getControllers' });
      console.log('UP controllers:', controllers);
      return controllers || [];
    } catch (err1) {
      console.log('up_getControllers method failed:', err1);
      ...
      try {
        ...
        console.log('LSP6 Key Manager:', keyManager);
        return [keyManager];
      } catch (err2) {
        console.log('Key Manager retrieval failed:', err2);
        return [];
      }
    }
  }
  ...
  const connect = async (): Promise<string[]> => {
    ...
    try {
      ...
      const executeWithRetry = async (
        ...
        try {
          console.log(`Attempt ${attemptNumber} to connect to LUKSO UP...`);
          return await fn();
        } catch (error: any) {
      ...
      try {
        if (window.lukso && window.lukso.contextAccounts) {
          ...
          console.log('Context accounts found:', ctxAccounts);
        ...
        console.warn('Error checking context accounts:', contextError);
      ...
      if (accounts.length > 0) {
        ...
        try {
          ...
          console.error('Error getting controller info:', controllerError);
      ...
      if (hasLuksoUP) {
        ...
        const pollForContextAccounts = () => {
          ...
          if (provider?.contextAccounts?.length > 0) {
            console.log('Found contextAccounts via provider polling:', provider.contextAccounts);
  ...
  useEffect(() => {
    if (typeof window.lukso !== 'undefined') {
      ...
      window.lukso.on?.('accountsChanged', async (accounts: string[]) => {
        console.log('LUKSO UP accounts changed:', accounts);
        setUpAccounts(accounts);
      ...
      window.lukso.on?.('chainChanged', (chainId: string) => {
        console.log('LUKSO UP chain changed:', chainId);
      });
      
      window.lukso.on?.('connect', (info: { chainId: string }) => {
        console.log('LUKSO UP connected:', info);
      });
      
      window.lukso.on?.('disconnect', (error: { code: number; message: string }) => {
        console.log('LUKSO UP disconnected:', error);
        setUpAccounts([]);
      });
